...
jQuery(document).on('click','.hip-pagination a',function (e) {
  ...
  if (pageToShow==="prev"){
    if (!isNaN(curPage) && parseInt(curPage)>1) {
      ...
      jQuery(this).parent().find("a[href='#" + (parseInt(curPage) - 1) + "']").addClass("active");
    }
  }
    ...
    jQuery(this).parent().find("a").removeClass("active");
    jQuery(this).parent().find("a[href='#" + parseInt(pageToShow) + "']").addClass("active");
  }
  ...
  items.filter(function() {
    ...
    if(typeof tags !== "undefined")
    {
      ...
      tags = tags.toLowerCase().split(" ");
       ...
       else if (jQuery(this).jsp().trim().toLowerCase().indexOf(value) > -1){
         init_filter++;
         jQuery(this).removeClass("hip-item-out");
       }
       else {
